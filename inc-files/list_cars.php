...
echo "<option value='0'>Всички коли</option>";
...
if (isset($_SESSION["username"]) && $_SESSION["usertype"]==1)
{
	...
	{
	echo "<tr>";
	echo "<td><a href='show_car.php?show_id=".$row['id_car']."' title='Подробна информация'>" .htmlspecialchars(stripslashes($row['make'])) . " </a></td><td>".htmlspecialchars(stripslashes($row['price'])). " лв.</td><td><a href='edit_car.php?edit_id=".$row['id_car']."' title='Промяна на цената и на допълнителната информация'>редактиране</a></td><td><a href='javascript:removeCar(".$row['id_car'].")'  title='Изтриване на данните'>изтриване</a></td>";
	echo "</tr>";
	}
}
else
